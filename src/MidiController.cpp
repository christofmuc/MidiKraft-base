...
void MidiController::handleIncomingMidiMessage(MidiInput* source, const MidiMessage& message) {
	...
	std::vector < std::function<void(MidiMessage const &)>> handlers;
	for (auto handler : handlerForNextMessage_) {
...
void MidiController::setNextEditBufferHandler(HandlerHandle const &handle, std::function<void(MidiMessage const &)> handler)
{
	handlerForNextMessage_.insert(std::make_pair(handle, handler));
}

bool MidiController::removeEditBufferHandler(HandlerHandle const &handle)
{
	if (handlerForNextMessage_.find(handle) != handlerForNextMessage_.end()) {
		handlerForNextMessage_.erase(handle);
		return true;
	}
	return false;
}
