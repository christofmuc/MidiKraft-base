...
namespace midikraft {
	...
	void FindSynthOnMidiNetwork::run()
	{
		...
		for (int input = 0; input < midiIns; input++) {
			auto inputInfo = MidiInput::getAvailableDevices()[input];
			MidiController::instance()->enableMidiInput(inputInfo);
		}
		...
		for (int output = 0; output < midiOuts; output++) {
			...
			if (synth_.needsChannelSpecificDetection()) {
				...
				for (int channel = 0; channel < 16; channel++) {
					...
					MidiController::instance()->getMidiOutput(juce::MidiOutput::getAvailableDevices()[output])->sendBlockOfMessagesFullSpeed(MidiHelpers::bufferFromMessages(detectMessage));
				}
			}
			else {
			...
			for (auto const &found : callback->locations()) {
				...
				if (synth_.endDeviceDetect(endDetectMessage)) {
					MidiController::instance()->getMidiOutput(juce::MidiOutput::getAvailableDevices()[output])->sendMessageNow(endDetectMessage);
				}
			}
		}
		...
		for (int input = 0; input < midiIns; input++) {
			auto inputInfo = MidiInput::getAvailableDevices()[input];
			MidiController::instance()->disableMidiInput(inputInfo);
		}
